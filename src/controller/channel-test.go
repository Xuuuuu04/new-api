...
func testChannel(channel *model.Channel, testModel string, endpointType string) testResult {
	...
	quota := int(math.Round((float64(totalTokens) * priceData.ModelPrice / 1_000_000.0) * common.QuotaPerUnit))
	tok := time.Now()
	milliseconds := tok.Sub(tik).Milliseconds()
	consumedTime := float64(milliseconds) / 1000.0
	other := service.GenerateTextOtherInfo(c, info, 1, 1, 1,
		0, 1, priceData.ModelPrice, priceData.GroupRatioInfo.GroupSpecialRatio)
