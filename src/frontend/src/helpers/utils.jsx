...
export const calculateModelPrice = ({
  ...
  // 2. 按每百万 tokens 定价
  const pricePer1MUsd = parseFloat(record.model_price || 0) * usedGroupRatio;
  if (!pricePer1MUsd) {
    return {
      price: '-',
  ...
  const pricePerUnit = pricePer1MUsd / unitDivisor;
  const displayVal = displayPrice(pricePerUnit);
    ...
    return (
      <>
        <span style={{ color: 'var(--semi-color-text-1)' }}>
          {t('单价')} {priceData.price} / 1{priceData.unitLabel}{' '}
          {t('tokens')}
        </span>
      </>
    );
